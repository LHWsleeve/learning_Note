...
- Java 堆可以处于物理上不连续的内存空间中，但逻辑上是连续的。但是对于大对象，例如数组对象，虚拟机常连续存放。
  ==**注意：JDK1.7随着方法区不再使用永久代来实现，而使用元空间之后。类的静态变量在类加载过程的准备节点会赋值并且随着Class对象一起放在Java堆当中，此时类变量放在方法区只是一种逻辑概念。**==
...
通过可达性分析算法来判断对象是否存活。算法的基本思路时通过一系列被叫“GC Root”的根对象作为起始**节点集**，从这些根节点开始根据引用关系乡下搜索(搜索的路劲被称之为"引用链")，如果某个对象 GC root 无法引用，则证明此对象不能再被使用。---**GC root实际上可以理解为任何活跃的引用**
**==GC root 有以下几种：==**
...
2. **软引用：** 用来描述一些还有用但是非必须的对象。只被软引用关联着的对象，**在系统将要发生内存溢出异常前，会把这些对象列进回收范围中做二次回收**，如果还不够，抛出内存溢出。==软引用可以和一个引用队列`(ReferenceQueue)`联合使用。如果软引用所引用对象被垃圾回收，JAVA虚拟机就会把这个软引用加入到与之关联的引用队列中。== 当内存不足时，JVM首先将软引用中的对象引用置为null，然后通知垃圾回收器进行回收。
3. **弱引用：** 比软引用强度更低，**仅能存活到下一次垃圾收集**发生为止。==同样，弱引用可以和一个引用队列(ReferenceQueue)联合使用，如果弱引用所引用的对象被垃圾回收，Java虚拟机就会把这个弱引用加入到与之关联的引用队列中==
...
当然，不可能保证每次存活对象都在  10%以内，所以当超出时survivor时，借助其他内存区域(老年代)进行分配担保。
...
==我可以认为，写屏障是虚拟机层面的AOP切面，在赋值的前后做卡表维护==
...
- **并发清理（Concurrent Cleanup）**：这个阶段用于清理那些==整个区域内连一个存活对象都没有找到的Region==（这类Region被称为Immediate Garbage Region）。
...
3. jmap：命令用于生产heap jump文件
4. jhat：与jmap搭配使用，用来分析生成的dumap，jhat内置了一个卫星的http/html服务器，生成dump的分析结果后，可以再浏览器中查看。
5. jstack： 生成当前时刻的线程快照
6. jinfo 实时查看和调整虚拟机参数

### 调优工具
1. jconsole 自带的java监控和管理控制台
2. jvisualvm，自带全能工具，分析内存快照，GC变化等
3. GChisto 一款专业分析gc日志的工具
### minor gc和full gc在什么时候发生
新生代不够的时候发生mgc，nvm内存不够的时候发生fgc
### JVM性能调优
1. 设定堆内存大小-Xmx：堆内存最大限制。
2. 设定新生代大小。 新生代不宜太小，否则会有大量对象涌入老年代
   -XX:NewSize：新生代大小
   -XX:NewRatio 新生代和老生代占比
   -XX:SurvivorRatio：eden空间和survivor空间的占比设
3. 垃圾回收器 
   年轻代用 -XX:+UseParNewGC 
...
1. **通过子类引用父类的静态字段，不会导致子类初始化**。==对于静态字段，只有直接定义这个字段的**类**才会被初始化。==(父类的final是和父类同时诞生的)
...
3. ==常量在编译阶段会存入调用类的常量池中，本质上并没有直接引用到定义常量的类==，因此不会触发定义常量的类的初始化。--**实际上对于final修饰的==基本类型==都是直接放入常量池，也就是不会触发该类的初始化。**
...
3. 当然还有"不通常"的情况，字段被修饰为 final 类型时候(必须是String或基本数据类型)，会变成==常类变量==，在准备阶段虚拟机就会为其赋值--即本阶段。(在编译阶段其实就已经放入常量池)
...
3. 初始化阶段：clinit()方法按照 static 顺序，收集静态变量，静态代码块，并初始化。
   ...
   此时 a=2,b=1
### 7.2.7 类的卸载
由Java虚拟机自带的类加载器所加载的类，在虚拟机的生命周期中，始终不会被卸载。比如Java虚拟机**自带的类加载器包括根类加载器、扩展类加载器和系统类加载器**。
ava虚拟机本身会始终引用这些类加载器，而这些类加载器则会始终引用它们所加载的类的Class对象，因此这些Class对象始终是可触及的。
...
**双亲委派优点**
1. **显而易见的好处**是保证Java中类随着他的类加载器一起具备了一种带有优先级的层次关系。例如无论哪个Object类，无论那个类加载器要加载Object，都会交给模型最顶端的启动类加载器进行加载。如果没有双亲委派机制，那么用户可以手写一个同名类区替换一个重要的类。
...
3. ==final 关键字的可见性==：被 final 修师的字段在构造器中一旦被初始化完成，并且构造器没有把`this`的引用传递出去，那么在其他线程中就能看见 final 字段的值。**(内存屏障)**---`this`逃逸是很严重的事情，其他线程可能会访问到**初始化了一半**(final类型变量，在类加载的准备阶段就会赋值，普通变量在初始化阶段才会赋初始值，之前都是默认值)的对象。
