...
- [JSP工作原理(忽略)](#jsp工作原理)
- [JSP有哪些内置对象、作用分别是什么(忽略)](#jsp有哪些内置对象、作用分别是什么)
...
- [JSP九大内置对象，七大动作，三大指令(忽略)](#jsp九大内置对象，七大动作，三大指令)
- [讲解JSP中的四种作用域(忽略)](#讲解jsp中的四种作用域)
...
在Java Web程序中，**Servlet**主要负责接收用户请求 `HttpServletRequest`,在`doGet()`,`doPost()`中做相应的处理，并将回应`HttpServletResponse`反馈给用户。**Servlet** 可以设置初始化参数，供Servlet内部使用。一个Servlet类只会有一个实例，在它初始化时调用`init()`方法，销毁时调用`destroy()`方法**。**Servlet需要在web.xml中配置，**一个Servlet可以设置多个URL访问**。**==Servlet不是线程安全==**，因此要谨慎使用类变量。
...
②get将表单中数据按照**name=value键值对**的形式，添加到action 所指向的URL 后面，并且两者使用"**?**"连接，而各个变量之间使用"**&**"连接；**post**是将表单中的数据**放在HTTP协议的请求头或消息体**中，传递到action所指向URL；

③get传输的数据要受到URL长度限制（最大长度是 2048 个字符，这其实是IE8的限制，chrome是2MB）；而post可以传输大量的数据，上传文件通常要使用post方式；
