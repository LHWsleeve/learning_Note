...
JDK1.8 之前 HashMap 由 数组+链表 组成的，数组是 HashMap 的主体，链表则是主要为了解决哈希冲突而存在的（“拉链法”解决冲突）.JDK1.8 以后在解决哈希冲突时有了较大的变化，==当链表长度大于阈值（默认为 8）时，并且数组总长度小于64==，将链表转化为红黑树，以减少搜索时间。
```
1、table长度足够，hash冲突过多
2、hash没有冲突，但是在计算table下标的时候，由于table长度太小，导致放在同一个位置
