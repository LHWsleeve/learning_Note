...
**JDK8，抛弃了原有的 Segment 分段锁(虽然代码里还有但是只在序列化的时候使用，应该是为了保证兼容性)，而采用了 CAS + synchronized(节点内变量使用volatile 修饰保证可见性) 来保证并发安全性。**
