...
2.  **TCP 协议** 位于传输层，提供可靠的**字节流服务**,TCP 协议为了更容易传送大数据才把数据分割， 而且 TCP 协议能够确认数据最终是否送达到对方。(**IP也会分割，为什么TCP也要分割呢？**)
 
**三次握手的一些问题**
   
   **为什么要三次握手？**
   >简单得讲因为三次握手才能保证双方具有接收和发送的能力。用于保证可靠性和流量控制维护的某些状态信息，这些信息的组合，包括**Socket、序列号和窗口大小** 称为连接。
   
   具体来说：
   1. **三次握手才可以避免历史连接（主要原因）**
